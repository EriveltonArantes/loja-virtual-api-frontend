...
const apiBaseUrl = "https://loja-virtual-api.onrender.com";
    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          <li>✓ Controle completo de Categoria Produto</li>
          <li>✓ Controle completo de Produto</li>
          <li>✓ Controle completo de Cliente</li>
          <li>✓ Controle completo de Pedido</li>
        </ul>
      </div>
...
function PainelCategoriaProduto({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["nome", "descricao", "icone", "ativo", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const carregar = () => fetch(apiBaseUrl + "/api/categoriaprodutos", { headers: authHeaders(token) })
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/categoriaprodutos/" + editId : apiBaseUrl + "/api/categoriaprodutos";
    fetch(url, {
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "descricao", "icone", "ativo"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/categoriaprodutos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
    ...
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "CategoriaProduto")}</div>
            
            <div className="item-meta-grid">
            <div className="item-field"><b>descricao</b><span>{item["descricao"] ?? "—"}</span></div>
            <div className="item-field"><b>icone</b><span>{item["icone"] ?? "—"}</span></div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} CategoriaProduto</h3>
...
function PainelProduto({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const [busca, setBusca] = React.useState("");
  const [pagina, setPagina] = React.useState(1);
  const [categoriaProdutoList, setCategoriaProdutoList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "descricao", "sku", "preco", "estoque", "estoqueMinimo", "categoriaId", "imagemUrl", "ativo", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/categoriaprodutos", { headers: authHeaders(token) }).then(r => r.json()).then(setCategoriaProdutoList).catch(() => {});
  }, [token]);
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "descricao", "sku", "preco", "estoque", "estoqueMinimo", "categoriaId", "imagemUrl", "ativo"].forEach(k => { f[k] = item[k] ?? ""; });
    ["categoria"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };
  ...
  return (
    <div>
        ...
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            <div className="item-photo-wrap">
              ...
              <img className="item-photo" style={{display: "none"}} src={item.imagemUrl} alt="Produto" loading="lazy" onLoad={e => { e.target.style.display = "block"; e.target.previousSibling.style.display = "none"; }} />
            </div>
            <div className="item-card-header">
              <div className="item-icon-badge">📦</div>
              <span className="item-id-tag">#{item.id}</span>
            ...
            <div className="item-meta-grid">
            <div className="item-field"><b>descricao</b><span>{item["descricao"] ?? "—"}</span></div>
            <div className="item-field"><b>sku</b><span>{item["sku"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>ativo</b><span>{item["ativo"] != null ? (item["ativo"] ? "Sim" : "Não") : "—"}</span></div>
            <div className="item-field"><b>categoria</b><span>{item.categoriaId ? ((categoriaProdutoList.find(o => o.id === item.categoriaId) || {}).nome ?? ("#" + item.categoriaId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      <label className="field-label">imagemUrl</label>
      <input type="file" className="upload-input" onChange={e => uploadArquivo(e.target.files[0], v => setForm(f => ({...f, imagemUrl: v})), token)} />
      {form["imagemUrl"] && <a className="upload-preview-link" href={apiBaseUrl + form["imagemUrl"]} target="_blank" rel="noreferrer">Ver arquivo enviado</a>}
      <label className="field-label checkbox-label">
        <input type="checkbox" checked={form["ativo"] === true || form["ativo"] === "true"} onChange={e => setForm({...form, ativo: e.target.checked})} />
        ativo
      </label>
      ...
      <select value={form["categoria"] ?? ""} onChange={e => setForm({...form, categoria: e.target.value})}>
        ...
        {(categoriaProdutoList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
function PainelCliente({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "email", "cpf", "telefone", "endereco", "cep", "cidade", "estado", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "email", "cpf", "telefone", "endereco", "cep", "cidade", "estado"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };
...
function PainelPedido({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["clienteId", "dataHora", "status", "valorTotal", "formaPagamento", "enderecoEntrega", "observacoes", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/pedidos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/pedidos/" + editId : apiBaseUrl + "/api/pedidos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["clienteId", "dataHora", "status", "valorTotal", "formaPagamento", "enderecoEntrega", "observacoes"].forEach(k => { f[k] = item[k] ?? ""; });
    ["cliente"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/pedidos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["status"] ?? "Pedido")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/pedidos/" + item.id, {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            <div className="item-photo-wrap">
              <div className="item-photo-fallback">P</div>
              <img className="item-photo" style={{display: "none"}} src={"https://picsum.photos/seed/pedido" + item.id + "/320/200"} alt="Pedido" loading="lazy" onLoad={e => { e.target.style.display = "block"; e.target.previousSibling.style.display = "none"; }} />
            </div>
            <div className="item-card-header">
              <div className="item-icon-badge">🧾</div>
              <span className="item-id-tag">#{item.id}</span>
            </div>
            <div className="item-title">{String(item["status"] ?? "Pedido")}</div>
            ...
            <div className="item-field"><b>enderecoEntrega</b><span>{item["enderecoEntrega"] ?? "—"}</span></div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Pedido</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
      ...
      <label className="field-label">enderecoEntrega</label>
      <input type="text" value={form["enderecoEntrega"] ?? ""} onChange={e => setForm({...form, enderecoEntrega: e.target.value})} />
...
export default function App() {
  ...
  const [aba, setAba] = React.useState("CategoriaProduto");
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Loja Virtual<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "CategoriaProduto" ? " active" : "")} onClick={() => setAba("CategoriaProduto")}><span className="nav-ico">📦</span>CategoriaProduto</button>
        ...
        <button className={"nav-btn" + (aba === "Pedido" ? " active" : "")} onClick={() => setAba("Pedido")}><span className="nav-ico">🧾</span>Pedido</button>
      ...
      <main className="main">
        ...
        {aba === "CategoriaProduto" && <PainelCategoriaProduto token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      ...
      {aba === "Pedido" && <PainelPedido token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      </main>
